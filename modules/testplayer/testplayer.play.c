...
static int dsp_write(struct auplay_st *st)
{
	...
	if (res != MMSYSERR_NOERROR)
		warning("testplayer: dsp_write: waveOutWrite: failed: %08x\n",
			res);
	else
		st->inuse++;
			     ...
			     unsigned int dev)
{
	...
	if (res != MMSYSERR_NOERROR) {
		warning("testplayer: waveOutOpen: failed %d\n", res);
		return EINVAL;
	}
...
int testplayer_play_alloc(struct auplay_st **stp, const struct auplay *ap,
